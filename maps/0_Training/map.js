...
function checkZoom(x, y) {
    ...
    if (inCenter && !zoomMode) {
        ...
        // Zoom mode is active - look up to zoom in, down to zoom out
        if (inUpperArea) {
            map.zoomIn();
            zoomMode = false;
...
function update() {
    try {
        ...
        if (legendContainer && !interactionLocked) {
            ...
            // Check each faculty tile to see if the user is looking at it
            Object.keys(facultyColors).forEach(faculty => {
                const tile = document.getElementById(`tile-${faculty}`);
                ...
                if (tile && bar) {
                    ...
                    if (isLooking) {
                        ...
                        // New tile - reset dwell timer
                        if (hoveredTileId !== faculty) {
                            hoveredTileId  = faculty;
                        ...
                        if (elapsed >= DWELL_TRIGGER_TIME) {
                            handleLayerToggle(faculty, tile, bar);
                        }
                    } else {
                        bar.style.width = '0%';
                    }
...
// Triggers the faculty highlight and locks interaction briefly to prevent re-triggering.
// ==========================================
function handleLayerToggle(faculty, tileElement, barElement) {
    console.log(`👁️ Layer toggled by gaze: ${faculty}`);

    // Call the highlight function
    toggleLayerHighlight(faculty, map);
...
// ==========================================
// 12. FACULTY HIGHLIGHT TOGGLE
// Selecting a faculty dims all other circles and shows only that faculty's labels.
// Selecting the same faculty again resets everything back to normal.
// ==========================================
let currentHighlightFaculty = null;

function toggleLayerHighlight(faculty, map) {
    // Toggle: selecting the same faculty again deselects it
    currentHighlightFaculty = (currentHighlightFaculty === faculty) ? null : faculty;

    // Update legend tile active state
    Object.keys(facultyColors).forEach(f => {
        const tile = document.getElementById(`tile-${f}`);
        if (tile) tile.classList.toggle('active', f === currentHighlightFaculty);
    });

    if (currentHighlightFaculty) {
        // Dim circles that don't belong to the selected faculty
        map.setPaintProperty('budovy-up', 'circle-opacity', [
            'case', ['==', ['get', 'FAKULTA'], currentHighlightFaculty], 1, 0.1
        ]);
        map.setPaintProperty('budovy-up', 'circle-stroke-opacity', [
            'case', ['==', ['get', 'FAKULTA'], currentHighlightFaculty], 1, 0.1
        ]);

        // Show labels only for the selected faculty's buildings
        map.setPaintProperty('budovy-up-labels', 'text-opacity', [
            'case', ['==', ['get', 'FAKULTA'], currentHighlightFaculty], 1, 0
        ]);
    } else {
        // Reset everything to default when no faculty is selected
        map.setPaintProperty('budovy-up', 'circle-opacity', 1);
        map.setPaintProperty('budovy-up', 'circle-stroke-opacity', 1);
        map.setPaintProperty('budovy-up-labels', 'text-opacity', 0); // Hide all labels again
    }
}

// ==========================================
// 13. KEYBOARD SHORTCUT: Return to start
...
map.on('load', async () => {
    ...
    // Load building data and generate legend
    await loadData(map);
