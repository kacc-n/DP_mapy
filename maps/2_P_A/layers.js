...
async function loadData(map) {
    for (const layer of myLayers) {
        ...
        try {
            ...
            console.log(`✅ Layer loaded (zoom 14+): ${layer.legend}`);
...
// Each tile shows a symbol matching the layer type and a dwell bar at the bottom.
// Tiles are activated by gaze dwell (no onclick - that is handled in map.js).
// ==========================================
function initEyeLegend(map) {
    ...
    myLayers.forEach(layer => {
        ...
        tile.id        = `tile-${id}`; // Used by map.js for gaze hit detection
...
// Called by map.js when a gaze dwell on a legend tile completes.
// Dims all other layers to 0.1 opacity and highlights the selected one.
// Selecting the same layer again resets everything to default.
// Restaurant labels are shown only when that layer is selected.
// ==========================================
function toggleLayerHighlight(layerId, map) {
    // Toggle: selecting the same layer again deselects it
